...
export default function RequestsPage() {
  ...
  const loadRequests = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/user/request/received', {
        credentials: 'include',
      });
  ...
  const handleAccept = async (requestId: string) => {
    try {
      const response = await fetch(`http://localhost:3001/request/review/accepted/${requestId}`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const handleReject = async (requestId: string) => {
    try {
      const response = await fetch(`http://localhost:3001/request/review/rejected/${requestId}`, {
        method: 'POST',
        credentials: 'include',
      });
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-lg text-gray-600 dark:text-gray-400 mb-6">
            Review and respond to people who liked your profile
          </p>
          
          {/* Stats */}
          <div className="flex items-center justify-center space-x-8 mb-8">
            <div className="text-center">
              <div className="text-3xl font-bold text-pink-600 dark:text-pink-400">{requests.length}</div>
              <div className="text-sm text-gray-600 dark:text-gray-400">Pending Requests</div>
            </div>
            <div className="w-px h-12 bg-gray-300 dark:bg-gray-600"></div>
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                {requests.length > 0 ? 'New' : 'None'}
              </div>
